...
contract SleepingBaseNftStaker is ReentrancyGuard {
    ...
    function getNftRarity(uint256[] memory tokenIds) private view returns (uint256[] memory rarityList){
        rarityList = new uint256[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            rarityList[i] = SleepingBase(stakingToken).tokenRarity(tokenIds[i]);
        }
    }
    /* ========== MUTATIVE FUNCTIONS ========== */
